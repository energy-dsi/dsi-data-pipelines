...
from mapping_function import schema_validation_trigger
...
def mapping_function(record: Record) ->  Record | list[Record] | None:
    ...
    try:
        finish_mapping = perf_counter()
        validation_result = schema_validation_trigger(data)
        ...
        if validation_result != True:
            data = dumps(
                validation_result, 
                default=lambda obj: obj.decode('utf-8') if isinstance(obj, bytes) else str(obj)
            )
        ...
        logger.info(f"Completed mapping of item in: {str(round(finish_mapping-start_mapping,3))}s")
        
        return mapped_record
    
    except Exception as e :
        logger.error("Error mapping object with exception {exp}".format(exp=e))  
