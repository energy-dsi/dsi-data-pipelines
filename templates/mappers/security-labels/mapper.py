...
default_security_label = {
    ...
    "containsPii": False,
    "dataSource": "dry-run-eq",
    "access": {
        "classification": "O",
        "allowedOrgs": ["org1", "org2"],
        "allowedNats": ["GBR"],
        "groups": []
    },
    "ownership":{
...
def get_headers(security_label):
    security_label = IDHModel(**security_label).build_security_labels()
    logger.info(f"security_label: {security_label}")

    output = RecordUtils.to_headers(
        {
            "Content-Type": "application/rdf+xml",
            "Security-Label": security_label,
            "policyInformation": { "IDH" : security_label },
        }
    )
    return output
...
def mapping_function(record: Record) ->  Record | list[Record] | None:
    ...
    mapped_record = Record(
        get_headers(default_security_label), 
        record.key, 
        data, 
        None
    )
